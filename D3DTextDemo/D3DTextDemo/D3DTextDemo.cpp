...
D3DTextDemo::D3DTextDemo() : solidColorVS_( 0 ) , solidColorPS_( 0 ) , inputLayout_( 0 ) , 
...
bool D3DTextDemo::DrawString( char * message , float startX , float startY )
{
	...
	float charWidth = 64.0f / 800.0f ;

	//Char's height on screen 
	float charHeight = 64.0f / 640.0f;

	//Char's texe1 width
	float texelWidth = 32.0f / 864.0f;

	//verts per-triangle(3)*total triangle(2) = 6
	...
	for( int i = 0; i < length; ++i )
	{
		...
		{
			//Grab one index past Z , which is blank space in the texture.
			texelWidth = ( indexZ - indexA ) + 1;
		}
