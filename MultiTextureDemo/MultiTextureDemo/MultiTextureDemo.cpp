...
MultiTextureDemo :: MultiTextureDemo() : effect_( 0 ) , inputLayout_( 0 ) , vertexBuffer_( 0 ) , 
	indexBuffer_( 0 ) , colorMap_( 0 ) , secondMap_( 0 ) , colorMapSampler_( 0 )
{
...
bool MultiTextureDemo::LoadContent()
{
	...
	{
		DXTRACE_MSG( "编译effect shader 失败！" );
		return false;
	}
	...
	if( FAILED( d3dResult ))
	{
		DXTRACE_MSG( "创建effect shader 失败！" );
...
void MultiTextureDemo::Render()
{
	...
	colorMapSampler = effect_ ->GetVariableByName( "colorMapSampler" ) ->AsSampler();
