...
TriangleDemo::TriangleDemo() : solidColorPS_( 0 ),solidColorVS_( 0 ),inputLayout_( 0 ),vertexBuffer_( 0 )
{
...
bool TriangleDemo::LoadContent( )
{
	...
	d3dResult = d3dDevice_ ->CreateVertexShader( vsBuffer ->GetBufferPointer() , vsBuffer ->GetBufferSize() , 0 , 
		&solidColorVS_ );
	...
	ZeroMemory( &resoureData , sizeof( resoureData ));
	resoureData.pSysMem = vertices;

	d3dResult = d3dDevice_->CreateBuffer( &vertexDesc , &resoureData , &vertexBuffer_ );
